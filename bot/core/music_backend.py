...
class MusicBackend:
    ...
    Priority: JioSaavn (licensed) → YT Music → SoundCloud → Audiomack → YouTube
    """
    ...
    async def search(self, query: str, limit: int = 5) -> List[Track]:
        ...
        # 1. Process JioSaavn results (Licensed catalog)
        ...
        # 2. Process YT Music results (high quality)
        if not isinstance(results[1], Exception):
            ytm_results = results[1]
        ...
        # 4. Process Audiomack results
        if not isinstance(results[3], Exception):
            am_results = results[3]
        ...
        # 5. Process YouTube results (last fallback)
        if not isinstance(results[4], Exception):
            yt_results = results[4]
